...
public class RangerSystemAccessControl
  ...
  final public static String RANGER_PRESTO_DEFAULT_HADOOP_CONF = "prestodb-ranger-site.xml";
  final public static String RANGER_PRESTO_SERVICETYPE = "prestodb";
  final public static String RANGER_PRESTO_APPID = "prestodb";
   ...
   */
  @Override
  public void checkCanCreateSchema(Identity identity, AccessControlContext context
  ...
  public void checkCanShowTablesMetadata(Identity identity, AccessControlContext context, CatalogSchemaName schema)
  {
    if (!hasPermission(createResource(schema.getCatalogName(), schema.getSchemaName()), identity, PrestoAccessType.SHOW)) {
      LOG.debug("RangerSystemAccessControl.ShowTablesMetadata(" + schema.getSchemaName() + ") denied");
  ...
  public void checkCanCreateViewWithSelectFromColumns(Identity identity, AccessControlContext context, CatalogSchemaTableName table, Set<String> columns) {
    try {
      ...
      AccessDeniedException.denyCreateViewWithSelect(table.getSchemaTableName().getTableName(), new Identity(identity.getUser(), identity.getPrincipal()));
    }
  }
